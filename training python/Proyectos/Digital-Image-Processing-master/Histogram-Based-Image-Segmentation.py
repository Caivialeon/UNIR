...
for i in range(0, len(onlyfiles1)):
    ...
    # applying smoothing
    orginalImg = cv.GaussianBlur(orginalImg, (7, 7), cv.BORDER_DEFAULT)
    # applying closing to remove hair
    kernel = np.ones((9, 9), np.uint8)
    orginalImg = cv.morphologyEx(orginalImg, cv.MORPH_CLOSE, kernel, iterations=2)
    # Here I am applying Histogram based Thresholding (cv.THRESH_BINARY_INV + cv.THRESH_OTSU) where i will have specific threshold value for each image
    ret, thresh = cv.threshold(orginalImg, 0, 255, cv.THRESH_BINARY_INV + cv.THRESH_OTSU)
    output = RemoveSmallRegions(thresh)
    kernel = np.ones((11, 11), np.uint8)
    output = cv.morphologyEx(output, cv.MORPH_CLOSE, kernel, iterations=2)
    DC = DiceCoefficient(output, actualImg)
    diceCoeff.append(DC)
    cv.imwrite(r'C:\Users\ZAIN UL ABAIDIN\Desktop\HSNew\IM' + strin[i] + '.bmp', output)
...
df1.to_excel('HistogramSegmentationDiceCoeff.xlsx')
